...
func (r Running) Calories() float64 {
	...
	return ((CaloriesMeanSpeedMultiplier*meanSpeed + CaloriesMeanSpeedShift) * r.Weight / MInKm * r.Duration.Hours() * MinInHours)
}
...
func (w Walking) Calories() float64 {
	meanSpeedMS := w.meanSpeed() * KmHInMsec
	heightM := w.Height / CmInM
	return ((CaloriesWeightMultiplier * w.Weight) + (math.Pow(meanSpeedMS, 2)/heightM)*CaloriesSpeedHeightMultiplier*w.Weight) * w.Duration.Hours() * MinInHours
}
